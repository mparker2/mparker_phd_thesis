...
Consensus sequence logos were generated using an in-house Python module `matplotlib_logo` (Appendix \ref{matplotlib_logo}). Unique splice junction pairs from spliced reads were identified, and the corresponding sequence information (8bp up and downstream of donor and acceptor) was extracted from the TAIR10 genome [@Initiative2000] using `pysam` [@Heger2014]. Position frequency matrices were generated from these sequences, and entropy score in bits was calculated and plotted. Code used to generate logos is available in Appendix \ref{exitron_splice_junction_logos}.
